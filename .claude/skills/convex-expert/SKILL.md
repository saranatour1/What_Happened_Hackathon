...
- SELF-VERIFY RULE — before declaring backend work done, verify it compiles and pushes: run `pnpm run typecheck` and push it to a deployment. Prefer the project's existing one; otherwise `pnpm exec convex dev --once` when `pnpm exec convex whoami` succeeds, and `CONVEX_AGENT_MODE=anonymous pnpm exec convex dev --once` ONLY when it does not. Forcing anonymous on a signed-in user rebinds `.env.local` and costs them the persistent, publishable cloud deployment they expect. Fix every error it reports before finishing — one verify round catches the wrong-relative-import / duplicate-symbol / unbalanced-paren class that otherwise breaks the deploy.
